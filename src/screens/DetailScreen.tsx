...
const DetailScreen: React.FC<Props> = ({route}) => {
  ...
  const detailedProduct = useAppSelector(
    (state: RootState) => state.products.detailedProduct,
  );
        ...
        {loading ? (
          ...
          <>
            ...
            <View style={styles.detailsContainer}>
              ...
              <Text style={styles.detail}>
                Availability: {detailedProduct?.availabilityStatus ?? '-'}
              </Text>
              <Text style={styles.detail}>
                Discount: {detailedProduct?.discountPercentage ?? '-'}%
              </Text>
              <Text style={styles.detail}>
                Dimensions:{' '}
                {`${detailedProduct?.dimensions?.width ?? '-'} x ${
                  detailedProduct?.dimensions?.height ?? '-'
                } x ${detailedProduct?.dimensions?.depth ?? '-'} cm`}
              </Text>
              <Text style={styles.detail}>
                Shipping: {detailedProduct?.shippingInformation ?? '-'}
              </Text>
              <Text style={styles.detail}>
                Return Policy: {detailedProduct?.returnPolicy ?? '-'}
              </Text>
            </View>
