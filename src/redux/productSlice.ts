...
export const fetchProducts = createAsyncThunk<ResponseProduct, number>(
  ...
  async (page: number) => {
    const response = await api.get(
      `/products?limit=10&skip=${(page - 1) * 10}&select=title,price,thumbnail`,
    );
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<ResponseProduct>) => {
      ...
      .addCase(
        ...
        (state, action: PayloadAction<DetailedProduct>) => {
